...
Think of a context type as different grammars of operation. Tasks are operative and have a clear finish line, regardless of context.

I may not know how much to water the plant. This is also its own context. We need to do research, and often with research we come out with new knowledge. It can be broader than a task because by the end we may find our initial inquiry was incomplete. We call this context type an *investigation*. There are more context types which will be summarized in the appendix: [[#4.1 Current Context Types|Current Context Types]].
...
Sequential reduction often happens on a need-be basis, it is about always asking "What is next?" which reduces the contention of creating many empty context files. 

Combined with context clusters, I often think of its core note as an originating context, and the peripherals arise as a consequence of sequential reduction of that context. 
...
You can think of sequential reduction as a mechanism of decomposing contexts and forming a context cluster from the core context or original intent.

# 6 Open Source Contribution Example

I strive to contribute to open source whenever I can. Often a motivating reason is an issue or feature that I want to have in my own projects. Using sequential reduction, I can follow a process that leads from my project feature all the way down to the filing of the issue and pull request made. Because I work in a stack fashion and I just return to the spawner once I'm done with a step, it is very anchoring of where I currently am with a project, and so it is easy to suspend and resume any step. This makes it possible to follow a process where I do not only accomplish my objectives, but also improve the open source ecosystem.
...
We explored the three levels of micro, meso, and macro progress in note taking. How each context file can have a journal with timestamped cards of progress or notes logged in, and how we can have context clusters with one originating intent context and many peripheral contexts that are spawned by sequential reduction.
