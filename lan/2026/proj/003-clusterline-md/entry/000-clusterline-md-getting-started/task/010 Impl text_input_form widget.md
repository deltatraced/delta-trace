...
Spawned in: [[lan/2026/proj/003-clusterline-md/entry/000-clusterline-md-getting-started/000-clusterline-md-getting-started#^spawn-task-fe35de|^spawn-task-fe35de]]

# Journal

2026-07-11 Wk 28 Sat - 01:19 +03:00

Just like `sb_options_filter_list` implemented through [[005 Add a custom fuzzy selector window with some text]] we want another basic widget: The text input form widget:
...
We need to also consider our prior policy decision [[000 InternalErrors must not be exposed to module consumers]]. So again the point is not exhaustive description of side effects. If a side effect occurs only on an invariant violation event, it must not be reported to module consumers.
