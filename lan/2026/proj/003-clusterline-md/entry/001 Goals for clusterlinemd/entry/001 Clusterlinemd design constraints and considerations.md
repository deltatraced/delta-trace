...
Spawned in: [[lan/2026/proj/003-clusterline-md/entry/001 Goals for clusterlinemd/001 Goals for clusterlinemd#^spawn-entry-f45b6b|^spawn-entry-f45b6b]]
...
2. Keep the default rename behavior, expect the user to use `Clusterline: fix` as a catch-all that will also rename these notes based on metadata.
...
3. Find a way to listen to a rename event, and complete the user's job by renaming the handles too.
